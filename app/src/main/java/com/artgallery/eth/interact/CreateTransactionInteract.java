package com.artgallery.eth.interact;
...
import com.artgallery.eth.repository.TokenRepository;
...
public class CreateTransactionInteract {
    ...
    public Single<String>  createEthTransaction(ETHWallet from,  String to, BigInteger amount, BigInteger gasPrice, BigInteger gasLimit, String password) {
        String rpcUrl = networkRepository.getEthNetWork().rpcServerUrl;
        final Web3j web3j = Web3j.build(new HttpService(rpcUrl));
    ...
    public Single<String>  createERC20Transfer(ETHWallet from,  String to, String contractAddress, BigInteger amount, BigInteger gasPrice, BigInteger gasLimit, String password) {
       String rpcUrl = from.getSymble().equalsIgnoreCase(C.QTS_SYMBOL)?networkRepository.getQtsNetWork().rpcServerUrl:networkRepository.getEthNetWork().rpcServerUrl;
        final Web3j web3j = Web3j.build(new HttpService(rpcUrl));
