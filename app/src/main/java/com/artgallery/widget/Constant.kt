package com.artgallery.widget

/**
 * Author:LC
 * Date:2018/11/22
 * Description:This is 圆形进度条属性默认值
 */
class Constant {

    companion object {
        val ANTI_ALIAS = false
        val SHADOW_SHOW = false
        val IS_GRADIENT = false
        val IS_ANIM=true
        val DEFALUT_DIGIT=2
        val SMALLCIRCLE_ENABLE = false
        val DEFAULT_CIR_WIDTH = 15f
        val DEFAULT_BGCIR_WIDTH = 14f
        val DEFAULT_ANIMTIME=1000
        val DEFAULT_MAX_VALUE=100F
        val DEFAULT_START_ANGLE=270F
        val DEFAULT_SWEEP_ANGLE=360F
        val DEFAULT_VALUE_SIZE=15F
        val DEFAULT_UNIT_SIZE=8F
        val DEFAULT_HINT_SIZE=10F
        val DEFAULT_SMALLCIRCLE_WIDTH=7F
        val DEFAULT_SHADOW_SIZE = 10F

    }
}